...
Evo.App = function () {
    /**
     * {Evo.App} Reference to this for this class
     */
    var _me = null;
    /**
    ...
    var _cfg = {
        ...
        energyColor  : 65280,
        /**
         * {Number} Amount of numbers in output buffer from organism,
         * which should be showed per one console.log() call. We don't
         * need to show every number, generated by echo command in
         * separate line, so we collect them into array and show
         * in bundle.
         */
        outSize      : 500
    };
    ...
    var _organisms = {};
    /**
     * {Object} Organisms output messages. Key - organism id, value - message
     */
    var _outs = {};
    /**
    ...
    /**
     * Formats specified value and converts it into readable string view
     * @param {*} v Value
    ...
    function _logSend(e, cmd, cfg) {
        ...
        console.log(data.id + ': ' + cmd + '(' + JSON.stringify(cfg) + ')' + ((data.resp + '') === '' ? '' : ':' + _formatValue(data.resp)));
    }
    /**
    ...
    /**
     * Just outputs a message from organism
     * @param {Number} v Number from organism
     * @param {Object} data RAW data from request
     * @private
     */
    function _echo(v, data) {
        var id = data.id;

        id = id.substr(0, id.indexOf('-'));
        if (!_outs[id]) {_outs[id] = [];}
        _outs[id].push(v);
        if (_outs[id].length > _cfg.outSize) {
            console.log('%corganism: %s, say: %s', 'color: #bada55', id, _outs[id].join());
            delete _outs[id];
        }
    }

    /**
     * Clones an organism into two.
     * @param {Array} body Current position of the organism
     * @param {Number} dir Direction of cloning. Means a position
     * near mother organism + 1 pixel. 0 - up, 1 - right, 2 -
     * bottom, 3 - left.
     * @param {Number} energy Amount of energy of current organism
     * @param {Uint16Array} code Code of current organism. Should
     * be the same for child organism
     */
    function _clone(body, dir, energy, code) {
        var x          = _dir2x(body[0], dir);
        var y          = _dir2y(body[1], dir);
        var nearEnergy = _world.getPixel(x, y);

        if (!nearEnergy) {
            _me.create({coordinates: [x,y], energy: energy, code: code, live: true});
        }
    }
    /**
    ...
    function _onMessage(e) {
        ...
        if (cmd && _api[cmd]) {
            return _api[cmd].apply(null, data.cfg.concat(data));
        }
